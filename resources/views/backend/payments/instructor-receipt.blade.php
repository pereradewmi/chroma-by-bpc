...
<head>
    <meta charset="utf-8">
    <title>Instructor Payment Receipt</title>
...
<div class="receipt-wrapper">
    @if($isPdf)
        ...
        <table class="pdf-table pdf-section">
            <tr>
                <td>
                    <div class="pdf-label">Instructor</div>
                    <div class="pdf-value">{{ $payment->instructor->tFName }} {{ $payment->instructor->tLName }}</div>
                    <div class="pdf-muted">ID: {{ $payment->instructor->T_ID }}</div>
                </td>
                <td class="pdf-right">
                    <div class="pdf-label">Month / Session</div>
                    <div class="pdf-value">{{ $months[$payment->month] ?? 'Unknown' }}</div>
                    <div class="pdf-muted">Session: {{ optional($payment->session)->sName }}</div>
                </td>
            </tr>
        </table>
        ...
        <table class="pdf-table pdf-section">
            <tr>
                <td>
                    <div class="pdf-label">Sessions</div>
                    <div class="pdf-muted">{{ $payment->sessions_count }} session(s)</div>
                    @if($payment->description)
                        <div class="pdf-muted">{{ $payment->description }}</div>
                    @endif
                </td>
                <td class="pdf-right">
    ...
    @else
        <div class="card receipt-card">
            <div class="card-body p-4">
                ...
                <div class="row mb-3">
                    <div class="col-8">
                        <h6 class="text-uppercase text-muted mb-1">Instructor</h6>
                        <p class="mb-0 font-weight-bold">{{ $payment->instructor->tFName }} {{ $payment->instructor->tLName }}</p>
                    ...
                    <div class="col-4 text-right">
                        ...
                        <small class="text-muted">Session: {{ optional($payment->session)->sName }}</small>
                    </div>
                </div>
                ...
                <div class="row mb-3">
                    <div class="col-8">
                        ...
                        <small class="text-muted">{{ $payment->sessions_count }} session(s)</small><br>
                        @if($payment->description)
