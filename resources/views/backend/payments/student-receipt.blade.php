...
<div class="receipt-wrapper">
    @if($isPdf)
        ...
        <table class="pdf-table pdf-section">
            <tr>
                <td>
                    <div class="pdf-label">Student</div>
                    <div class="pdf-value">{{ $payment->student->fName }} {{ $payment->student->lName }}</div>
                    <div class="pdf-muted">ID: {{ $payment->student->AutoID }}</div>
                </td>
                <td class="pdf-right">
                    <div class="pdf-label">Class / Month</div>
                    <div class="pdf-value">{{ $payment->classRoom->cName }}</div>
                    <div class="pdf-muted">Month: {{ $months[$payment->month] ?? 'Unknown' }}</div>
                </td>
            </tr>
        </table>
        ...
        <table class="pdf-table pdf-section">
            <tr>
                <td>
                    <div class="pdf-label">Payment Type</div>
                    <div class="pdf-muted text-uppercase">
                ...
                <td class="pdf-right">
                    ...
                    <div class="pdf-amount">Rs. {{ number_format($payment->classRoom->classfee ?? 0, 2) }}</div>
                </td>
            </tr>
        </table>
            ...
            <div class="card-body p-4">
                ...
                <div class="d-flex justify-content-between align-items-center">
                    ...
                    @if(!request()->has('download'))
                        <a href="{{ route('payments.receipt', ['id' => $payment->paymentID]) }}?download=1" class="btn btn-sm btn-primary no-print">Download PDF</a>
                    @endif
                </div>
            </div>
        </div>
